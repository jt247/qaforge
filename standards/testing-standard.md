...
## Standards mapping
The security checks in this standard are QA-level product security hygiene (access boundaries, session lifecycle, obvious data exposure), not a penetration test. ASVS and WSTG are used as a vocabulary and a source of test methods for those checks, not as a claim of assessment against the full standard. Security requirements target applicable OWASP ASVS 5.0.0 controls, with WSTG v4.2 testing methods. Record exact control IDs only after consulting the source; do not claim full coverage from a short checklist. Accessibility target: WCAG 2.2 AA where applicable; automated checks alone are insufficient.
